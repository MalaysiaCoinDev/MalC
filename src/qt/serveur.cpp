...
* Foundation, Inc., 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 MALC*/
#include <QScrollBar>
#include "serveur.h"
        QStringList users;
        bool delist = true;
...
void Serveur::errorSocket(QAbstractSocket::SocketError error)
{
	switch(error)
	{
		case QAbstractSocket::HostNotFoundError:
            affichage->append(tr("<em>ERROR : can't find MALCCoin server.</em>"));
			break;
...
{
    affichage->append("Connecting... to MALCCoin IRC. Please wait.");

	sendData("USER "+pseudo+" localhost "+serveur+" :"+pseudo);
    sendData("NICK "+pseudo);
    affichage->append("Connected to MALCCoin IRC.");
