    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->MALCCoinAvailableLabel->setPalette(sample_palette);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "TOTAL(BTC)"<< "MALCCoin(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)"  << "MALCCoin(SIZE)" << "TOTAL(BTC)");
    /*OrderBook Table Init*/
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(MALCCoin)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //MALCCoinst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=BTC-MALCCoin");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-MALCCoin&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-MALCCoin&count=100");
      return Response;
}
...
QString tradingDialog::BuyMALCCoin(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-MALCCoin&quantity=";
...
QString tradingDialog::SellMALCCoin(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=MALCCoin-BTC";
...
QString tradingDialog::GetBalance(QString Currency){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getbalance?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=MALCCoin";
            URL += Currency;
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=MALCCoin-BTC&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>MALCCoin Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> MALCCoin");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    double MALCCoinSupply = 0;
    double MALCCoinDemand = 0;
    double BtcSupply  = 0;
    double BtcDemand  = 0;
    ...
    foreach (const QJsonValue & value, SellArray)
    {
        ...
        MALCCoinSupply = MALCCoinSupply + y;
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        MALCCoinDemand = MALCCoinDemand + y;
        ...
        ui->MALCCoinSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(MALCCoinSupply,'i',8) + "</span><b> MALCCoin</b>");
        ...
        ui->MALCCoinDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(MALCCoinDemand,'i',8) + "</span><b> MALCCoin</b>");
    ...
    switch (index){
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       QString balance = GetBalance("MALCCoin");
                                       QString str;
                                       QJsonObject ResultObject =  GetResultObjectFromJSONObject(balance);

                                       ui->MALCCoinAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("BTC");
                         ...
                         Response = GetBalance("MALCCoin");

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->MALCCoinBalanceLabel,*ui->MALCCoinAvailableLabel,*ui->MALCCoinPendingLabel, QString::fromUtf8("MALCCoin"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputMALCCoin->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
{
    //calculate amount of BTC that can be gained from selling MALCCoin available balance
    QString responseA = GetBalance("MALCCoin");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableMALCCoin = ResultObject["Available"].toDouble();

    ui->UnitsInputMALCCoin->setText(str.number(AvailableMALCCoin,'i',8));
}
...
void tradingDialog::on_BuyMALCCoin_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyMALCCoin(Order,Quantity,Rate);
...
void tradingDialog::on_SellMALCCoinBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputMALCCoin->text().toDouble();
    ...
    QString Msg = "Are you sure you want to Sell ";
            Msg += ui->UnitsInputMALCCoin->text();
            Msg += " MALCCoin @ ";
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellMALCCoin(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputMALCCoin_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
